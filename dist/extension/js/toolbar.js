...
class LocalStorageService {
    ...
    removeItem(key) {
        chrome.storage.local.remove([ `${PREFIX}${key}-${this.hostname}`, `latest-${PREFIX}${key}` ]);
    }
}
...
const ColorMode = Object.freeze({
    ...
    COLOR_DARK_BG: "darkBgColor",
    COLOR_LIGHT_BG: "lightBgColor"
});
...
const COLOR_READ_ACTIONS = {
    ...
    darkBgColor: ColorMode.COLOR_DARK_BG,
    lightBgColor: ColorMode.COLOR_LIGHT_BG
};
...
class ColorReadService {
    ...
    parseSettingValue(value) {
        ...
        const mode = COLOR_READ_ACTIONS[action];
    ...
    resolveProfile() {
        ...
        if (this.mode === ColorMode.COLOR_DARK_BG) {
            return this.buildDarkBgProfile();
        }
        if (this.mode === ColorMode.COLOR_LIGHT_BG) {
            return this.buildLightBgProfile();
        }
...
class ColorReadComponent extends AbstractSetting {
    activesValues={
        values: DEFAULT_VALUE + "splitSyllables_word," + "colorSyllables_word," + "darkBgColor_word," + "lightBgColor_word",
        valueSelected: 0
    };
    constructor() {
        super();
...
class ColorProfileRuleComponent extends HTMLElement {
    ...
    renderRule() {
        ...
        this.querySelector(`#${PREFIX}color-read-profile-rule-example`).textContent = Array.isArray(this.rule.example) ? this.rule.example.join(", ") : this.rule.example;
        const phoneticEl = this.querySelector(`#${PREFIX}color-read-profile-rule-phonetics`);
        const exampleEl = this.querySelector(`#${PREFIX}color-read-profile-rule-example`);
...
class ColorProfileComponent extends HTMLElement {
    ...
    renderProfile=profileJson => {
        ...
        let rules = profile.process?.flatMap((step => step.format ?? [])) ?? [];
...
class EditColorReadComponent extends HTMLElement {
    ...
    colorReadActions=[ "none", "splitSyllables", "colorSyllables", "darkBgColor", "lightBgColor" ];
