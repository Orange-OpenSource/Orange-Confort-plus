...
chrome.action.onClicked.addListener(async (tab) => {
	...
	if ((!isEnabled && !isInjected) || (isEnabled && !isInjected)) {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
	if (['edge:', 'chrome:', 'about:'].some(browser => tab.url?.startsWith(browser))) {
		...
		const reloads = await chrome.storage.local.get(`${PREFIX}is-reload-${tab.id}`);
		const isReload = reloads[`${PREFIX}is-reload-${tab.id}`];
		...
		if (changeInfo.url || isReload) {
			if (isEnabled) {
				...
				chrome.storage.local.set({ [`${PREFIX}is-reload-${tabId}`]: false });
			}
...
chrome.webNavigation.onCommitted.addListener((details) => {
	if (details.transitionType === 'reload') {
		chrome.storage.local.set({ [`${PREFIX}is-reload-${details.tabId}`]: true });
	}
});
